...
X_train_one, X_test_one, Y_train_one, Y_test_one = train_test_split(
    X_train_tfidf_one, aa['label'].values, test_size=0.25, random_state=1000)
X_train_two, X_test_two, Y_train_two, Y_test_two = train_test_split(
    X_train_tfidf_two, aa['label'].values, test_size=0.25, random_state=1000)
X_train_three, X_test_three, Y_train_three, Y_test_three = train_test_split(
    X_train_tfidf_three, aa['label'].values, test_size=0.25, random_state=1000)
...
print(metrics.classification_report(Y_test_one,
                                    predicted_one, target_names=['ham', 'spam']))

print('for the bigram we have:')
print(metrics.classification_report(Y_test_two,
                                    predicted_two, target_names=['ham', 'spam']))

print('for the trigram we have:')
print(metrics.classification_report(Y_test_three,
                                    predicted_three, target_names=['ham', 'spam']))
